          ...
          <Tabs defaultActiveKey="1">
            <Tabs.TabPane tab="Aujourd'hui" key="1">
              <h1 className="font-bold text-center">
                ...
                <span className="text-xl">
                  {listeRecette.length !== 0
                    ? listeRecette.data.reduce((acc, v) => {
            ...
            <Tabs.TabPane tab="A une date" key="2">
              ...
              <h1 className="font-bold text-center mt-12">
                Montant récette :{" "}
                <span className="text-xl">
                  {listeRecetteDate.data.reduce((acc, v) => {
                    return acc + v.MontantPayeUSD;
                  }, 0)}
                </span>
                USD
              ...
              <table className="table-auto w-full items-center">
                ...
                <tbody>
                  {listeRecetteDate.data.map((item, i) => {
                    return (
                      <tr
                        key={i}
                        className="bg-white h-10 border-b border-slate-100 hover:text-orange-400 hover:bg-slate-200 font-bold"
            ...
            <Tabs.TabPane tab="Par Agent" key="4">
              <h1 className="text-center">
                Période :&nbsp;
                <input
                  type="date"
                  value={searchByAgentInput1}
                  onChange={(e) => {
                    setSearchByAgentInput1(e.target.value);
                  }}
                  className="bg-gray-300 outline-none h-10 px-4 ml-3"
                />
                <input
                  type="date"
                  value={searchByAgentInput2}
                  onChange={(e) => {
                    setSearchByAgentInput2(e.target.value);
                  }}
                  className="bg-gray-300 outline-none h-10 px-4 ml-3"
                />
                <select onChange={(e)=>{setPercepteur(e.target.value)}} className="h-10 outline-none w-content border-1 border-gray-500 ml-2">
                  <option value="">--Selectionner un agent</option>
                  {
                    percepteurs.map((item,i)=>{
                      return (<option value={item.id}>{item.nom}</option>)
                    })
                  }
                </select>
                <button
                  onClick={() => recetteParAgent()}
                  className="bg-blue-700 hover:bg-blue-400 hover:shadow-md h-10 px-2 ml-2 text-white"
                >
                  voir
                </button>
              ...
              <h1 className="font-bold text-center">
                Montant récette :{" "}
                <span className="text-xl">
                  {listeRecetteAgent.data.reduce((acc, v) => {
                    return acc + v.MontantPayeUSD;
                  }, 0)}
                </span>
                USD
              ...
              <table className="table-auto w-full items-center">
                ...
                <tbody>
                  {listeRecetteAgent.data.map((item, i) => {
                    return (
                      <tr
                        key={i}
                        className="bg-white h-10 border-b border-slate-100 hover:text-orange-400 hover:bg-slate-200 font-bold"
