...
import Vehiculestationne from "./Vehiculestationne";
...
const Home = () => {
  ...
  const validerPaye=()=>{
   // alert(ticket);
    ...
    if(deviseSelected=="USD")
    {
      donnees.append("mtUSD",mtPayeUSD);
      donnees.append("mtCDF",0);
    }else
    {
      donnees.append("mtUSD",0);
      donnees.append("mtCDF",parseFloat(mtPayeUSD)*parseFloat(taux));
    }
    fetch(api,{method:"POST",body:donnees}).then(r=>r.json())
    .then(r=>{
      if(r.n==1)
  ...
  const validerPaye2=(_ticket,_mt)=>{
   // alert(ticket);
    
    let donnees=new FormData();
    donnees.append("qry","validerPaiement");
    donnees.append("ticket",_ticket);
    donnees.append("idUser",idUser);
    if(deviseSelected=="USD")
    {
      donnees.append("mtUSD",_mt);
      donnees.append("mtCDF",0);
    }else
    {
      donnees.append("mtUSD",0);
      donnees.append("mtCDF",parseFloat(_mt)*parseFloat(taux));
    }
    fetch(api,{method:"POST",body:donnees}).then(r=>r.json())
    .then(r=>{
      if(r.n==1)
      {
          s.value="";
        window.open(endPoint+"facture.php?fact="+_ticket);
        //window.location.reload();
        /*Modal_antd.success({
          centered:true,
          okText:"Continuer",
  ...
  const hasScanned=(e)=>{
    ...
    }else
    {
      ...
      newListeVehicules=listeVehicules.filter(t=>{
        let nbreCart=e.length;
        return t.Id==e.toUpperCase();
      });
      //alert(newListeVehicules.length);
      //new Promise()
      setVehicules(newListeVehicules.length);
      setListeVehiculesFiltered(newListeVehicules);
      setIsSearch(true);
    ...
    <h4 className="text-center bg-gray-50 rounded-md text-xl py-2 pl-3 text-blue-300">        
      ...
      <div className="">
        ...
        <div className="mb-2 flex flex-row justify-center">
          <input type="text" 
            ref={s}
            onChange={(e)=>{hasScanned(e.target.value)}}
            className="w-72 outline-none placeholder:text-white py-2 rounded-sm px-1 bg-green-500 text-white" placeholder="Veuillez scanner le QRCODE"/>
